...
if defined _int (
REM echo Checking Internet Connection            [Connected%ping_f%]
) else (
set error=1
call :dk_color %Red% "Checking Internet Connection            [Not Connected]"
call :dk_color %Blue% "Internet is required for Windows Activation."
)
...
if !errorlevel! EQU 0 (
REM echo Changing Windows Region To USA          [Successful] [Script will change it back]
) else (
...
REM echo Generating GenuineTicket.xml            [Successful]
)
...
if defined _perm (
echo:
call :dk_color %Green% "%winos% is activated with a Volume license KEY."
goto :dl_final
)
...
echo:
if defined _perm (
call :dk_color %Green% "%winos% is permanently activated with a Volume license Key."
) else (
...
if !errorlevel! EQU 0 (
REM echo Restoring Windows Region                [Successful]
) else (
...
if defined generickey (set "keyecho=Setting Volume Product Key         ") else (set "keyecho=Installing Product Key                 ")
if %keyerror% EQU 0 (
