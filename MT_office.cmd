...
echo:
REM echo Activating Office...                    [C2R ^| %_version% ^| %_oArch%]
...
echo:
REM echo Activating Office...                    [C2R ^| %_version% %_AudienceData%^| %_oArch%]
...
call :dk_color %Green% "Office Volume Key is Set to server, Office Is activated."
...
REM echo Help: %mas%troubleshoot
) else (
...
REM echo Modifying Hash of Custom sppcs.dll      [Successful]
)
)
...
echo:
REM echo Activating Office...                    [MSI ^| %_version% ^| %_oArch%]
...
REM echo Clearing Office License Blocks          [Successfully cleared from all %counter% user accounts]
...
REM echo Adding Registry to Skip License Check   [Successfully added to all %counter% ^& future new user accounts]
)
...
if defined generickey (set "keyecho=Volume Product Key         ") else (set "keyecho=Installing Product Key                 ")
if %keyerror% EQU 0 (
...
REM echo Checking WPA Registry Count             [%wpainfo%]
)
)
